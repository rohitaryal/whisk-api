...
if (!cookie) throw new Error("we need cookie for testing");

const whisk = new Whisk(cookie);

test.if(!!cookie)("Generate caption", async () => {
...
test.if(!!cookie)("Generate multiple caption", async () => {
    const result = await Whisk.generateCaption(
        await imageToBase64("assets/banner.jpeg"),
        whisk.account, 8
    );
...
test.if(!!cookie)("Get media information", async () => {
...
test.if(!!cookie)("Create new project", async () => {
...
test.if(!!cookie)("Generate image", async () => {
    const result = await whisk.newProject("STUPID PROJECT");
...
test.if(!!cookie)("Generate image with parameters", async () => {
...
test.if(!!cookie)("Refine image", async () => {
    const result = await whisk.newProject();
...
test.if(!!cookie)("Nested image refinement", async () => {
    const result = await whisk.newProject();
